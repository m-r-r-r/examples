---
title: "`{r} params$county` 2024 BRFSS District Profile – Adult Health"
...
state_result <-
  county_result_raw %>%
  filter(county == "State") %>%
  pivot_wider(values_from = Percent, names_from = county) %>%
  select(Variable, 
         State)
...
risk_fact_result <-
  county_result_tidy %>%
  filter(group == "risk_fact" & county == params$county) %>%
  select("Indicator",
...
prevent_behav_result <-
  county_result_tidy %>%
  filter(group == "prevent_behav" & county == params$county) %>%
  select("Indicator",
         "District Most Recent %",
         "Vermont Most Recent %",
         "Estimated # of Adults in District",
...
sdoh_result <-
  county_result_tidy %>%
  filter(group == "sdoh" & county == params$county) %>%
  select("Indicator",
         "District Most Recent %",
         "Vermont Most Recent %",
         "Estimated # of Adults in District",
...
chronic_cond_result <-
  county_result_tidy %>%
  filter(group == "chronic_cond" & county == params$county) %>%
  select("Indicator",
         "District Most Recent %",
         "Vermont Most Recent %",
         "Estimated # of Adults in District",
...
demo_result <-
  county_result_tidy %>%
  filter(group == "demo" & county == params$county) %>%
  select("Indicator",
         "District Most Recent %",
         "Vermont Most Recent %",
         "Estimated # of Adults in District",
